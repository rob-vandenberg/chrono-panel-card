...
// ─── storage() decorator, vendored from HA frontend ───────────────────────
// HA's own src/common/decorators/storage.ts is what hui-card-picker and
// hui-stack-card-editor use for their clipboard property, so a write
// through this exact mechanism lands in the same shared in-memory cache
// those components read from - a plain sessionStorage.setItem() call does
// not (see v2.0.36 history above). home-assistant-frontend is not
// published to npm/unpkg - HA's own files import this via a relative path
// from inside their own monorepo, which isn't reachable from an externally
// loaded third-party card - so the real implementation is ported here
// directly instead, faithfully, rather than guessed at or simplified.
class ChronoStorageClass {
      ...
      if (ev.key && this.hasKey(ev.key)) {
        ...
        if (this._listeners[ev.key]) {
          this._listeners[ev.key].forEach((listener) =>
            listener(ev.oldValue ? JSON.parse(ev.oldValue) : ev.oldValue, this._storage[ev.key])
          );
        }
      }
    });
  ...
  addFromStorage(storageKey) {
    if (!this._storage[storageKey]) {
      ...
      if (data) this._storage[storageKey] = JSON.parse(data);
    }
  }
  ...
  subscribeChanges(storageKey, callback) {
    ...
    return () => this.unsubscribeChanges(storageKey, callback);
  }

  unsubscribeChanges(storageKey, callback) {
    if (!(storageKey in this._listeners)) return;
    const index = this._listeners[storageKey].indexOf(callback);
    if (index !== -1) this._listeners[storageKey].splice(index, 1);
  }
...
function storage(options) {
  return (proto, propertyKey) => {
    ...
    let storageInstance;
    if (storageName && storageName in chronoStorages) {
      storageInstance = chronoStorages[storageName];
    } else {
      storageInstance = new ChronoStorageClass(window[storageName]);
      chronoStorages[storageName] = storageInstance;
    }
    ...
    const subscribeChanges =
      options.subscribe !== false
        ? (el) =>
            storageInstance.subscribeChanges(storageKey, (oldValue) => {
    ...
    const setValue = (el, value) => {
      let oldValue;
      if (options.state) oldValue = getValue();
      storageInstance.setValue(storageKey, options.serializer ? options.serializer(value) : value);
      if (options.state) el.requestUpdate(propertyKey, oldValue);
    };
    ...
    if (options.subscribe) {
      ...
      proto.connectedCallback = function () {
        ...
        if (!this.__unsubLocalStorage) {
          this.__unsubLocalStorage = subscribeChanges?.(this);
        }
      };
      proto.disconnectedCallback = function () {
        disconnectedCallback.call(this);
        this.__unsubLocalStorage?.();
        this.__unsubLocalStorage = undefined;
      };
    }
...
class ChronoPanelCardEditor extends LitElement {
  ...
  // Writes through HA's own real storage() mechanism for the
  // "dashboardCardClipboard" sessionStorage key, borrowed directly off the
  // live hui-card-picker class rather than a vendored copy of the
  // decorator - this lands in the exact same shared in-memory cache
  // hui-card-picker itself reads from (confirmed: same prototype, same
  // singleton), not a separate one. That alone isn't sufficient though:
  // an already-rendered hui-card-picker only redraws its "Paste from
  // Clipboard" tile when something calls requestUpdate() on that specific
  // picker instance - writing the shared cache doesn't do that by itself,
  // since the borrowed setter's own requestUpdate() call fires on
  // whichever element it's called with as `this` (here, this editor), not
  // on the picker. So after writing, explicitly find any hui-card-picker
  // currently rendered in our own shadow root and poke it directly.
  _copy() {
    const value = JSON.parse(JSON.stringify(this._config.cards[this._selected]));
    const PickerClass = customElements.get("hui-card-picker");
    const desc = PickerClass && Object.getOwnPropertyDescriptor(PickerClass.prototype, "_clipboard");

    if (desc && desc.set) {
      const oldValue = desc.get ? desc.get.call(this) : undefined;
      desc.set.call(this, value);

      const picker = this.shadowRoot?.querySelector("hui-card-picker");
      if (picker) {
        picker.requestUpdate("_clipboard", oldValue);
      }
    } else {
      // hui-card-picker isn't reachable for some reason - fall back to our
      // own storage() mechanism so Copy still does something useful,
      // even though a currently-open picker won't pick it up live.
  ...
  _renderCardEditor(cards, numCards) {
    ...
    return html`
      ...
      ${this._renderContent(cardConfig)}
    `;
  }
